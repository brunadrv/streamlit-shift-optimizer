...
def display_rollup_table(rollup_data, selected_dates):
    """Display the rollup table using AG-Grid with alternating department colors and date columns"""
    from st_aggrid import AgGrid, GridOptionsBuilder, GridUpdateMode, DataReturnMode, JsCode
    
    df = pd.DataFrame(rollup_data)
    
    # Get list of departments for alternating colors
    departments = list(st.session_state.departments.keys())
    
    # Row style with alternating department colors and total row highlighting
    row_style = JsCode(f"""
    function(params) {{
        ...
        if (dept && dept.includes('- Total')) {{
            style['background-color'] = '#d9e9f2';
            style['font-weight'] = '600';
            style['border-top'] = '2px solid #999';
            return style;
        }}
        
        // Alternating colors for department blocks
        ...
        if (deptIndex % 2 === 0) {{
            style['background-color'] = '#f5f5f5';  // Light gray
        }} else {{
