...
def create_combined_hc_attendance_aggrid_table(filtered_hc_data, filtered_attendance_data, expected_hc_total):
    ...
    function(params) {
        ...
        if (field === 'S1_Scheduled') {
            return params.data['S1_Tooltip'] || '';
        } else if (field === 'S2_Scheduled') {
            return params.data['S2_Tooltip'] || '';
        } else if (field === 'S3_Scheduled') {
            return params.data['S3_Tooltip'] || '';
        }
...
def main():
    ...
    <div style="display: flex; gap: 20px; margin: 10px 0 20px 0; font-size: 13px; color: #666;">
        <div style="display: flex; align-items: center; gap: 8px;">
            <div style="width: 20px; height: 20px; border: 1px solid #ddd; background-color: #ffcccc;"></div>
            <span>>20% variance from last week</span>
        </div>
        <div style="display: flex; align-items: center; gap: 8px;">
            <div style="width: 20px; height: 20px; border: 1px solid #ddd; background-color: #ffe6cc;"></div>
            <span>10-20% variance from last week</span>
        </div>
        <div style="display: flex; align-items: center; gap: 8px;">
            <span>✏️ Click cells in the hedge row to edit (type 10 or -5, % added automatically and calculated with FTE/TEMP Attendance Assumption)</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
