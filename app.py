...
def filter_data_by_selections(data_dict, selected_date, shifts):
    """Filter transposed data based on date and shift selections"""
    filtered_data = {}
    
    # Convert selected_date to string format that matches our keys
    date_str = selected_date.strftime("%Y-%m-%d")
    
    for key, value in data_dict.items():
        # Parse the key format: "2026-02-12 Thu Shift 1"
        parts = key.split(' ')
        if len(parts) >= 4:
            key_date = parts[0]  # "2026-02-12"
            key_shift = parts[-1]  # "1", "2", "3"
            
            # Check if this entry matches our filters
            date_matches = (key_date == date_str)
            shift_matches = (key_shift in [s[0] for s in shifts])  # Convert "1st" -> "1"
            
            if date_matches and shift_matches:
...
def main():
    ...
    filtered_shift_data = filter_data_by_selections(st.session_state.shift_summary_transposed, selected_date, shifts)
    filtered_hc_data = filter_data_by_selections(st.session_state.weekly_hc_details_transposed, selected_date, shifts)
    filtered_attendance_data = filter_data_by_selections(st.session_state.attendance_assumptions_transposed, selected_date, shifts)
    filtered_employees = filter_employee_data_by_selections(st.session_state.employee_data, selected_department, week, shifts)
    ...
    if current_view == "Shift Summary":
        ...
        fig = create_transposed_shift_summary_table_with_tooltips(filtered_shift_data)
        ...
        # Use the transposed HC Details table for Roster HC Summary with filtered data
        fig = create_transposed_hc_details_table_with_tooltips(filtered_hc_data)
        st.plotly_chart(fig, use_container_width=True)
        ...
        st.plotly_chart(create_transposed_attendance_assumptions_table(filtered_attendance_data), use_container_width=True)
