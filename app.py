...
def generate_shift_breakdown(filtered_data, metric_key):
    """Generate shift-level breakdown cards similar to reference image"""
    ...
    for key, data in filtered_data.items():
        parts = key.split(' ')
        if len(parts) >= 4:
            shift_num = parts[-1]  # Get shift number
            value = data.get(metric_key, 0)
            
            # For gap, calculate as expected - needed
            if metric_key == 'gap':
