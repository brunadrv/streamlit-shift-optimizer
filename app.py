...
def create_combined_headcount_attendance_table(filtered_hc_data, filtered_attendance_data):
    ...
    for emp_type in employee_types:
        ...
        for shift in shifts:
            ...
            def get_cell_class(value, is_percentage=False):
                cell_class = "data-cell"
                if is_percentage:
                    # For attendance percentages, check variance (mock logic)
                    try:
                        rate = float(str(value).replace('%', ''))
                        if rate >= 90:  # High variance mock
                            cell_class += " high-variance"
                        elif rate >= 80:  # Medium variance mock
                            cell_class += " medium-variance"
                        # No low-variance class needed
                    except:
                        pass
                else:
                    # For headcount numbers
                    if isinstance(value, (int, float)) and value > 0:
                        if value >= 40:  # High variance (>20%)
                            cell_class += " high-variance"
                        elif value >= 20:  # Medium variance (10-20%)
                            cell_class += " medium-variance"
                        # No coloring for <10% variance
                return cell_class
            
            # Headcount cell
            hc_cell_class = get_cell_class(headcount, False)
            ...
            # Attendance cell
            att_cell_class = get_cell_class(attendance_clean, True)
