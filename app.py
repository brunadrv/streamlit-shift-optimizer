...
def generate_shift_breakdown(filtered_shift_data, metric_key):
    """Generate shift-level breakdown with vertical alignment and lighter gray badges"""
    ...
    # Sort days chronologically by date, not alphabetically
    ...
    if not sorted_days or not sorted_shifts:
        return '<div style="color: #999; font-style: italic;">No shift data available</div>'
    
    # Create vertical table structure
    html_content = '<div style="display: inline-block; width: 100%;">'
    
    # Header row with day names
    html_content += '<div style="display: flex; margin-bottom: 4px;">'
    html_content += '<div style="width: 40px;"></div>'  # Empty space for shift labels
    for day, _ in sorted_days:
        html_content += f'<div style="flex: 1; text-align: center; font-weight: 600; color: #555; font-size: 0.8rem;">{day}</div>'
    html_content += '</div>'
    
    # Shift rows
    for shift_num in sorted_shifts:
        html_content += '<div style="display: flex; margin-bottom: 3px; align-items: center;">'
        html_content += f'<div style="width: 40px; font-size: 0.75rem; color: #666; font-weight: 500;">S{shift_num}</div>'
        
        for day, day_data in sorted_days:
            value = day_data['shifts'].get(shift_num, '—')
            # Use lighter gray color #adb5bd instead of #6c757d
            badge_html = f'<span style="background-color: #adb5bd; color: white; padding: 2px 8px; border-radius: 10px; font-size: 0.75rem; margin: 0 auto; display: block; text-align: center; font-weight: 500; width: fit-content;">{value}</span>'
            html_content += f'<div style="flex: 1; text-align: center;">{badge_html}</div>'
