...
def main():
    ...
    # Four toggle buttons with built-in highlighting - Clean solution
    view_options = ["📊 Shift Summary", "👥 Roster HC Summary", "📋 Roster HC Details", "📈 Attendance Assumption"]
    view_keys = ["Shift Summary", "Roster HC Summary", "Roster HC Details", "Attendance Assumption"]
    
    # Find current selection index
    try:
        current_index = view_keys.index(st.session_state.current_view)
    except ValueError:
        current_index = 0
        st.session_state.current_view = "Shift Summary"
    ...
    selected_index = view_options.index(selected_option)
    st.session_state.current_view = view_keys[selected_index]
