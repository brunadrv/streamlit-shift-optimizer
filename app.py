...
def calculate_dynamic_metrics(location, department, week, selected_date, shifts):
    ...
    gap_change = int((week_mult - 0.88) * 100 - 2)  # Gap change vs last week
    
    # Calculate punches based on expected HC
    punches = int(expected * 1.3)  # Punches are typically 30% higher
    punches_change = int((week_mult - 0.92) * 100 + 5)  # Punches change vs last week
...
def get_gap_status_info(gap, gap_percentage):
    ...
    if gap == 0:
        return "âœ… Fully Staffed", "gap-neutral", "#28a745"
    elif gap > 0:
        return f"â†˜ï¸ Understaffed by {abs(gap_percentage)}%", "gap-negative", "#dc3545"
    else:
        return f"â†—ï¸ Overstaffed by {abs(gap_percentage)}%", "gap-positive", "#28a745"
...
def main():
    ...
    # Key metrics row - All four KPIs: Needed HC, Expected HC, Gap in HC, Punches
    ...
    with col4:
        render_metric_with_tooltip("Punches", "Actual punches recorded")
        st.markdown(f'<div class="metric-large">{metrics["punches"]}</div>', unsafe_allow_html=True)
        
        # Dynamic change indicator for punches
        punches_change_symbol = "↗️" if metrics["punches_change"] >= 0 else "↘️"
        punches_change_class = "metric-change-up" if metrics["punches_change"] >= 0 else "metric-change-down"
        st.markdown(f'<div class="metric-change {punches_change_class}">{punches_change_symbol} {abs(metrics["punches_change"])}% vs last week</div>', unsafe_allow_html=True)
