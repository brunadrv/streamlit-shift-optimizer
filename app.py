...
def generate_shift_breakdown(filtered_shift_data, metric_key):
    """Generate shift-level breakdown with modern badge-style formatting"""
    ...
    for shift_key in sorted(filtered_shift_data.keys()):
        ...
        if len(parts) >= 4:  # Ensure proper format: YYYY-MM-DD Day Shift N
            ...
            if day not in day_groups:
                day_groups[day] = []
            day_groups[day].append((shift_num, shift_value))
    
    # Format each day's shifts with badge styling
    day_lines = []
    for day in sorted(day_groups.keys()):
        shifts = sorted(day_groups[day], key=lambda x: int(x[0]) if x[0].isdigit() else 0)
        
        # Create styled badges for each shift
        shift_badges = []
        for shift_num, value in shifts:
            # Color-code based on metric type for visual appeal
            badge_color = get_shift_badge_color(metric_key, value)
            badge_html = f'<span style="background-color: {badge_color}; color: white; padding: 2px 6px; border-radius: 10px; font-size: 0.75rem; margin: 0 2px; font-weight: 500;">S{shift_num}: {value}</span>'
            shift_badges.append(badge_html)
        
        # Format day line with icon and badges
        day_icon = "📅" if len(day_groups) > 1 else ""  # Only show icon if multiple days
        day_line = f'<div style="margin: 2px 0;"><span style="font-weight: 600; color: #555; margin-right: 6px;">{day_icon} {day}</span>{"".join(shift_badges)}</div>'
...
def get_shift_badge_color(metric_key, value):
    """Get appropriate badge color based on metric type and value"""
    if 'Gap' in metric_key:
        # Gap metric: red for negative (shortage), green for positive (overstaffed), gray for zero
        if isinstance(value, (int, float)):
            if value < 0:
                return '#dc3545'  # Red for shortage
            elif value > 0:
                return '#28a745'  # Green for overstaffed  
            else:
                return '#6c757d'  # Gray for balanced
        return '#6c757d'
    elif 'Punches' in metric_key:
        return '#17a2b8'  # Teal for punches
    elif 'Expected' in metric_key:
        return '#fd7e14'  # Orange for expected
    elif 'Needed' in metric_key:
        return '#6f42c1'  # Purple for needed
    else:
...
def main():
    ...
    with col1:
        ...
        # Shift breakdown directly below widget with enhanced styling
