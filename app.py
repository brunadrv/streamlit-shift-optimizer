...
def main():
    ...
    # Create perfectly aligned editable Hedge Attendance Rate row
    columns_list = sorted(list(filtered_hc_data.keys()))
    
    # Calculate column widths to match the table structure
    # First column is Employee Type (wider), then pairs of Scheduled HC / Attendance columns
    col_widths = [1.5] + [1] * len(columns_list)
    cols = st.columns(col_widths)
    
    # Row label column
    with cols[0]:
        st.markdown("""
            <div style='background-color: #fff3cd; padding: 8px; border: 1px solid #ddd; 
                        font-weight: bold; font-style: italic; text-align: left; min-height: 35px;
                        display: flex; align-items: center;'>
                Hedge Attendance Rate (+/-)
            </div>
        """, unsafe_allow_html=True)
    
    # Create input fields aligned with each date/shift column
    hedge_changes = False
    for idx, shift_key in enumerate(columns_list, 1):
        parts = shift_key.split(' ')
        date_str = parts[0]
        shift_num = parts[-1]
        hedge_key = f"{date_str}_{shift_num}st"
        ...
        with cols[idx]:
            current_hedge = st.session_state.hedge_rates.get(hedge_key, 0.0)
            new_hedge = st.number_input(
                f"Hedge for {date_str} Shift {shift_num}",
                value=current_hedge,
                min_value=-100.0,
                max_value=100.0,
                step=0.5,
                format="%.1f",
                label_visibility="collapsed",
                key=f"hedge_input_{hedge_key}",
                help=f"Adjust attendance rate for {date_str} Shift {shift_num} (applies to FTE and TEMP only)"
            )
            
            if new_hedge != current_hedge:
                st.session_state.hedge_rates[hedge_key] = new_hedge
                hedge_changes = True
    
    # Rerun if changes detected
    if hedge_changes:
        st.rerun()
    
    # Display Total Expected HC row info
    st.markdown("""
        <div style='background-color: #e8f4f8; padding: 8px; border: 1px solid #ddd; 
                    font-weight: bold; text-align: left; margin-top: -1px;'>
            Total Expected HC (calculated with hedge rates applied to FTE and TEMP)
        </div>
    """, unsafe_allow_html=True)
    
    # Optional: Clear all hedges button
    if st.session_state.hedge_rates and any(v != 0 for v in st.session_state.hedge_rates.values()):
        col1, col2 = st.columns([4, 1])
        with col2:
            if st.button("🗑️ Clear All Hedges", key="clear_all_hedges", type="secondary"):
                st.session_state.hedge_rates = {}
