...
def create_combined_hc_attendance_aggrid_table(filtered_hc_data, filtered_attendance_data, expected_hc_total):
    ...
    function(params) {
        ...
        try {
            ...
            if (Object.keys(cohortData).length > 0) {
                for (var key in cohortData) {
                    ...
                    if (val !== 0) {
                        cohortLines.push('  - ' + (val > 0 ? '+' : '') + val + ' ' + key);
                    }
                }
            }
        } catch (e) {
        ...
        // Build compact tooltip with line breaks
        var sign = absolute > 0 ? '+' : '';
        var direction = absolute > 0 ? 'more than' : 'less than';
        
        var result = sign + absolute + ' headcount ' + direction + ' last week.';
