...
def create_combined_headcount_attendance_table(filtered_hc_data, filtered_attendance_data, selected_department):
    ...
    for emp_type in employee_types:
        ...
        for shift in shifts:
            ...
            try:
                ...
                # Initialize shift totals on first pass
    ...
    # Total row (use Overview expected values for consistency)
    html_content += f"<tr><td class='row-header total-row'>Total Expected HC</td>"
    for shift in shifts:
        # Use the Overview expected value to maintain consistency
        total = shift_expected_from_overview.get(shift, shift_totals.get(shift, 0))
        tooltip_total = f"<strong>Total Expected HC</strong><br>{shift}<br><br>Expected: {total}<br>(From Overview calculation)"
