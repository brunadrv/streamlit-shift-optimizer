...
def create_transposed_shift_summary_table_with_tooltips():
    ...
    fig.update_layout(
        height=300,
        margin=dict(l=0, r=0, t=20, b=-10),  # Reduced bottom margin to close gap
...
def create_transposed_hc_details_table_with_tooltips():
    ...
    fig.update_layout(
        height=350,
        margin=dict(l=0, r=0, t=20, b=-10),  # Reduced bottom margin
...
def main():
    ...
    if current_view == "Shift Summary":
        ...
        with st.expander("📊 View Detailed Shift Breakdowns"):
            ...
            # Create a comprehensive table for all shifts
            shifts_data = []
            for shift_key, data in st.session_state.shift_summary_transposed.items():
                # Parse shift information
                parts = shift_key.split(' ')
                date = parts[0]  # 2026-02-12
                day = parts[1]   # Thu
                shift = parts[-1]  # Shift 1 -> 1
                shift_num = shift.split(' ')[-1]
                
                shifts_data.append({
                    'Date': date,
                    'Day': day,
                    'Shift': shift_num,
                    'Date & Shift': f"{date} {day} Shift {shift_num}",
                    'Total Needed': data['Total Needed'],
                    'Total Expected': data['Total Expected'],
                    'Total Gap': data['Total Gap'],
                    'Attendance %': data['Total Attendance Assumption'],
                    'Total Punches': data['Total Punches'],
                    'Needed Breakdown': data['tooltip_needed'],
                    'Expected Change': data['tooltip_expected'],
                    'Gap Change': data['tooltip_gap'],
                    'Attendance Change': data['tooltip_attendance'],
                    'Punches Change': data['tooltip_punches']
                })
            
            # Display as organized sections by date
            current_date = None
            for shift_data in shifts_data:
                if shift_data['Date'] != current_date:
                    current_date = shift_data['Date']
                    st.markdown(f"### {current_date} - {shift_data['Day']}")
                
                # Create columns for each shift
                col1, col2 = st.columns([1, 2])
                
                with col1:
                    st.markdown(f"**Shift {shift_data['Shift']} Summary**")
                    
                    # Summary metrics table
                    summary_df = pd.DataFrame([
                        ['Total Needed', shift_data['Total Needed']],
                        ['Total Expected', shift_data['Total Expected']],
                        ['Gap', shift_data['Total Gap']],
                        ['Attendance %', shift_data['Attendance %']],
                        ['Total Punches', shift_data['Total Punches']]
                    ], columns=['Metric', 'Value'])
                    
                    st.dataframe(summary_df, hide_index=True, use_container_width=True)
                
                with col2:
                    st.markdown(f"**Shift {shift_data['Shift']} Details & Changes**")
                    
                    # Details table
                    details_df = pd.DataFrame([
                        ['Needed Breakdown', shift_data['Needed Breakdown']],
                        ['Expected vs Last Week', shift_data['Expected Change']],
                        ['Gap vs Last Week', shift_data['Gap Change']],
                        ['Attendance vs Last Week', shift_data['Attendance Change']],
                        ['Punches vs Last Week', shift_data['Punches Change']]
                    ], columns=['Detail Type', 'Information'])
        ...
        with st.expander("👥 View Detailed HC Breakdowns"):
            ...
            # Create comprehensive HC data
            hc_data = []
            for shift_key, data in st.session_state.weekly_hc_details_transposed.items():
                parts = shift_key.split(' ')
                date = parts[0]
                day = parts[1] 
        ...
        with st.expander("📈 View Detailed Attendance Breakdowns"):
            ...
            attendance_data = []
            for shift_key, data in st.session_state.attendance_assumptions_transposed.items():
                parts = shift_key.split(' ')
                date = parts[0]
                day = parts[1]
                shift_num = parts[-1].split(' ')[-1]
