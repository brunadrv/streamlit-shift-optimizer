...
def create_combined_hc_attendance_table(filtered_hc_data, filtered_attendance_data, expected_hc_total):
    ...
    html_content += "<tr>"
    html_content += "<td class='row-header hedge-row'>Hedge Attendance Rate (+/-)</td>"
...
def main():
    ...
    with st.expander("✏️ **Edit Hedge Attendance Rates**", expanded=False):
        ...
        hedge_data = []
        for shift_key in sorted(list(filtered_hc_data.keys())):
            parts = shift_key.split(' ')
            date_str = parts[0]
            day_abbr = parts[1]
            shift_num = parts[-1]
        ...
        # Column config
        column_config = {
            "Date": st.column_config.Column("🔒 Date", disabled=True, width="medium"),
            "Day": st.column_config.Column("🔒 Day", disabled=True, width="small"),
            "Shift": st.column_config.Column("🔒 Shift", disabled=True, width="small"),
            "Hedge Rate (%)": st.column_config.NumberColumn(
                "✏️ Hedge Rate (%)",
                help="Enter +/- percentage (e.g., -5.0 reduces attendance by 5%)",
                min_value=-100.0,
                max_value=100.0,
                step=0.5,
                format="%.1f",
        ...
        edited_hedge_df = st.data_editor(
            ...
            hide_index=True,
            num_rows="fixed",
            key="hedge_rate_editor"
        )
        
        # Detect changes
        changes_made = False
        for idx, row in edited_hedge_df.iterrows():
            hedge_key = f"{row['Date']}_{row['Shift']}st"
            new_value = row['Hedge Rate (%)']
            old_value = st.session_state.hedge_rates.get(hedge_key, 0.0)
            
            if new_value != old_value:
                st.session_state.hedge_rates[hedge_key] = new_value
                changes_made = True
        
        if changes_made:
            st.success("✓ Hedge rates updated! Table refreshed.")
            st.rerun()
        ...
        with col2:
            if st.session_state.hedge_rates and any(v != 0 for v in st.session_state.hedge_rates.values()):
                if st.button("🗑️ Clear All", key="clear_all_hedges"):
                    st.session_state.hedge_rates = {}
