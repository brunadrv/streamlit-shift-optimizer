    ...
    try:
        ...
        for date in selected_dates:
            if date in week_data:
                for shift in shifts:
                    if shift in week_data[date]:
                        key = f"{date.strftime('%Y-%m-%d')} {date.strftime('%a')} Shift {shift[-1]}"
                        filtered_data[key] = week_data[date][shift]
        
        return filtered_data
    except Exception as e:
        # Return empty data if any filtering fails
        return {}
...
def create_combined_headcount_attendance_table(filtered_data):
    """Create combined table responsive to filtered data"""
    
    if not filtered_data:
        return "<div style='text-align: center; padding: 20px; color: #666;'>No data available for selected filters</div>"
    
    # Create table structure from filtered data
    html_content = """
    ...
    # Dynamic column headers based on filtered data
    shift_keys = list(filtered_data.keys())
    for key in shift_keys:
        html_content += f"<th colspan='2'>{key}</th>"
    ...
    for emp_type in employee_types:
        ...
        for shift_key in shift_keys:
            ...
            data_key_map = {
                ...
                'NEW HIRES': 'new_hires',
                'Day Labor (Flex)': 'flex',
                'Day Labor (WW/GS)': 'wwgs',
                'Overtime (VEH/MEH)': 'veh',
                'Time Off (PTO)': 'pto'
            }
            ...
            # Get headcount and attendance values
            ...
            html_content += f"""
                <td class='data-cell'>{headcount}</td>
                <td class='data-cell'>{attendance}</td>
            """
...
def main():
    ...
    with col4:
        ...
        def get_production_week_range(week_str):
            ...
            # Correct production week mappings (Thursday start dates verified for 2026)
            week_mappings = {
                "2026-W06": datetime(2026, 2, 5),   # Thu Feb 5 - Wed Feb 11
                "2026-W07": datetime(2026, 2, 12),  # Thu Feb 12 - Wed Feb 18  
                "2026-W08": datetime(2026, 2, 19),  # Thu Feb 19 - Wed Feb 25
                "2026-W09": datetime(2026, 2, 26),  # Thu Feb 26 - Wed Mar 4
                "2026-W10": datetime(2026, 3, 5)    # Thu Mar 5 - Wed Mar 11
            }
            
            thursday_start = week_mappings.get(week_str, datetime(2026, 2, 13))
            
            # Verify it's actually a Thursday and adjust if needed
            # Monday=0, Tuesday=1, Wednesday=2, Thursday=3, Friday=4, Saturday=5, Sunday=6
            days_until_thursday = (3 - thursday_start.weekday()) % 7
            if days_until_thursday != 0:
                thursday_start = thursday_start + timedelta(days=days_until_thursday)
            
            # Wednesday is 6 days after Thursday
            wednesday_end = thursday_start + timedelta(days=6)
            
            return thursday_start, wednesday_end
    ...
    with col2:
        ...
        # CHANGE 1: Add shift breakdown under Expected HC (responsive to filters)
        def generate_shift_breakdown_html(filtered_data):
            if not filtered_data:
                return '<div style="color: #999; font-style: italic;">No data for selected filters</div>'
            
            # Group by date and shift
            breakdown_html = '<div style="background-color: #f8f9fa; border-radius: 6px; padding: 8px; margin-top: 8px;">'
            
            dates_processed = set()
            for key, data in filtered_data.items():
                # Extract date from key: "2026-02-19 Thu Shift 1"
                parts = key.split(' ')
                if len(parts) >= 4:
                    date_str = parts[0]
                    day_str = parts[1]
                    ...
                    if date_str not in dates_processed:
                        ...
                        breakdown_html += f'<div style="margin-bottom: 4px; font-size: 0.75rem; color: #666;">{day_str}</div>'
                        
                        # Find all shifts for this date
                        date_shifts = []
                        for k, d in filtered_data.items():
                            if k.startswith(date_str):
                                shift_n = k.split()[-1]
                                expected = d.get('expected', 0)
                                date_shifts.append(f'''
                                    <span style="background-color: #e3f2fd; color: #1565c0; padding: 2px 6px; border-radius: 12px; font-size: 0.7rem; margin: 0 2px; display: inline-block; min-width: 20px; text-align: center; font-weight: 500;">
                                        S{shift_n}<br>{expected}
                                    </span>
                                ''')
                        
                        if date_shifts:
                            breakdown_html += f'<div style="margin-bottom: 6px;">{"".join(date_shifts)}</div>'
            
            breakdown_html += '</div>'
            return breakdown_html
        
        shift_breakdown_html = generate_shift_breakdown_html(filtered_data)
    ...
    with col4:
        # CHANGE 4: Add Total Punches pill (now responsive)
    ...
    # CHANGE 5 & 6: Combined table (now responsive to all filters)
    html_table = create_combined_headcount_attendance_table(filtered_data)
