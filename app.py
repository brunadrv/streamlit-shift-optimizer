...
def create_attendance_html_table_with_tooltips(filtered_attendance_data):
    """Create HTML table with tooltips for attendance assumption data"""
    ...
    shifts = []
    for shift_key in sorted(filtered_attendance_data.keys()):
        parts = shift_key.split(' ')
        date = parts[0]
        day = parts[1]
        shift_num = parts[-1].split(' ')[-1]
        shifts.append((date, day, shift_num, shift_key))
    ...
    html_content += "<th>FTE Attendance</th>"
    html_content += "<th>TEMP Attendance</th>"
    html_content += "<th>NEW HIRES Rate</th>"
    html_content += "<th>FLEX Rate</th>"
    html_content += "<th>WW/GS Rate</th>"
    html_content += "<th>VEH Rate</th>"
    html_content += "</tr>"
    
    # Data rows
    for date, day, shift_num, shift_key in shifts:
        data = filtered_attendance_data[shift_key]
        html_content += "<tr>"
        
        # Row header
        html_content += f"<td class='row-header'>{date} {day}<br>Shift {shift_num}</td>"
        
        # Data cells with tooltips
        metrics = [
            ('FTE Attendance Assumption', data['FTE Attendance Assumption'], data.get('tooltip_fte', '')),
            ('TEMP Attendance Assumption', data['TEMP Attendance Assumption'], data.get('tooltip_temp', '')),
            ('NEW HIRES Show Up Rate', data['NEW HIRES Show Up Rate'], data.get('tooltip_newhires', '')),
            ('FLEX Show Up Rate', data['FLEX Show Up Rate'], data.get('tooltip_flex', '')),
            ('WW/GS Show Up Rate', data['WW/GS Show Up Rate'], data.get('tooltip_wwgs', '')),
            ('VEH Show Up Rate', data['VEH Show Up Rate'], data.get('tooltip_veh', ''))
        ]
        
        for metric_name, value, tooltip in metrics:
            # Create detailed tooltip content from expandable section
            tooltip_content = f"<strong>{metric_name}</strong><br>{date} {day} - Shift {shift_num}<br><br>"
            ...
            if 'FTE' in metric_name:
                tooltip_content += "• Full-time employee attendance rate<br>"
                tooltip_content += "• Based on historical performance<br>"
                tooltip_content += "• Core workforce reliability metric"
            elif 'TEMP' in metric_name:
                tooltip_content += "• Temporary worker attendance rate<br>"
                tooltip_content += "• Generally more variable than FTE<br>"
                tooltip_content += "• Seasonal/contract workforce metric"
            elif 'NEW HIRES' in metric_name:
                tooltip_content += "• New employee show-up rate<br>"
                tooltip_content += "• Typically lower during adjustment period<br>"
                tooltip_content += "• Training and onboarding consideration"
            elif 'FLEX' in metric_name:
                tooltip_content += "• Flexible worker show-up rate<br>"
                tooltip_content += "• On-demand workforce availability<br>"
                tooltip_content += "• Peak coverage support"
            elif 'WW/GS' in metric_name:
                tooltip_content += "• Warehouse/General Support rate<br>"
                tooltip_content += "• Cross-functional team reliability<br>"
                tooltip_content += "• Support operations coverage"
            elif 'VEH' in metric_name:
                tooltip_content += "• Vehicle/Equipment operator rate<br>"
                tooltip_content += "• Specialized role reliability<br>"
                tooltip_content += "• Critical operations support"
            ...
            if '%' in str(value):
                try:
                    ...
                    if rate >= 90:
                        tooltip_content += "<br><br>🟢 <strong>Excellent</strong> - High reliability"
                    elif rate >= 80:
                        tooltip_content += "<br><br>🟡 <strong>Good</strong> - Acceptable performance"
                    elif rate >= 70:
                        tooltip_content += "<br><br>🟠 <strong>Fair</strong> - Needs improvement"
                    else:
                        tooltip_content += "<br><br>🔴 <strong>Poor</strong> - Immediate attention needed"
                except:
                    pass
...
def create_roster_hc_html_table_with_tooltips(filtered_hc_data):
    """Create HTML table with tooltips for roster HC data"""
    ...
    # Build the data structure
    shifts = []
    for shift_key in sorted(filtered_hc_data.keys()):
        parts = shift_key.split(' ')
        date = parts[0]
        day = parts[1]
        shift_num = parts[-1].split(' ')[-1]
    ...
    # Header row
    html_content += "<tr><th>Date & Shift</th>"
    html_content += "<th>FTE</th>"
    html_content += "<th>Part Time</th>"
    html_content += "<th>Temp</th>"
    html_content += "<th>Total HC</th>"
    html_content += "</tr>"
    ...
    for date, day, shift_num, shift_key in shifts:
        ...
        # Data cells with tooltips
        metrics = [
            ('FTE', data['FTE'], data.get('tooltip_fte', '')),
            ('Part Time', data['Part Time'], data.get('tooltip_part_time', '')),
            ('Temp', data['Temp'], data.get('tooltip_temp', '')),
            ('Total HC', data['Total HC'], data.get('tooltip_total_hc', ''))
        ]
        
        for metric_name, value, tooltip in metrics:
            # Create detailed tooltip content from expandable section
            tooltip_content = f"<strong>{metric_name} Details</strong><br>{date} {day} - Shift {shift_num}<br><br>"
            ...
            # Additional context from expandable details
            tooltip_content += "<br><br><strong>Additional Information:</strong><br>"
            tooltip_content += f"• {metric_name} headcount for this shift<br>"
            
            if 'FTE' in metric_name:
                tooltip_content += "• Full-time equivalent employees<br>"
                tooltip_content += "• Permanent staff with benefits<br>"
                tooltip_content += "• Core workforce foundation"
            elif 'Part Time' in metric_name:
                tooltip_content += "• Part-time employees<br>"
                tooltip_content += "• Flexible scheduling workforce<br>"
                tooltip_content += "• Helps cover peak hours"
            elif 'Temp' in metric_name:
                tooltip_content += "• Temporary workers<br>"
                tooltip_content += "• Contract or seasonal staff<br>"
                tooltip_content += "• Variable demand coverage"
            elif 'Total HC' in metric_name:
                fte_val = data.get('FTE', 0)
                pt_val = data.get('Part Time', 0) 
                temp_val = data.get('Temp', 0)
                tooltip_content += f"• Breakdown: {fte_val} FTE + {pt_val} PT + {temp_val} Temp<br>"
                tooltip_content += "• Complete workforce count<br>"
                tooltip_content += "• All employee categories combined"
...
def create_shift_summary_html_table_with_tooltips(filtered_shift_data):
    """Create HTML table with tooltips for shift summary data"""
    ...
    # Header row
    html_content += "<tr><th>Date & Shift</th>"
    html_content += "<th>Total Needed</th>"
    html_content += "<th>Total Expected</th>"
    html_content += "<th>Total Gap</th>"
    html_content += "<th>Attendance Assumption</th>"
    html_content += "<th>Total Punches</th>"
    html_content += "</tr>"
    ...
    for date, day, shift_num, shift_key in shifts:
        ...
        # Data cells with tooltips
        metrics = [
            ('Total Needed', data['Total Needed'], data.get('tooltip_needed', '')),
            ('Total Expected', data['Total Expected'], data.get('tooltip_expected', '')),
            ('Total Gap', data['Total Gap'], data.get('tooltip_gap', '')),
            ('Total Attendance Assumption', data['Total Attendance Assumption'], data.get('tooltip_attendance', '')),
            ('Total Punches', data['Total Punches'], data.get('tooltip_punches', ''))
        ]
        ...
        for metric_name, value, tooltip in metrics:
            ...
            if 'Gap' in metric_name and str(value).replace('-', '').isdigit():
                gap_val = int(value) if str(value).replace('-', '').isdigit() else 0
                if gap_val > 0:
                    tooltip_content += "<br><br>⚠️ <strong>Staffing shortage</strong> - Additional staff needed"
                elif gap_val < 0:
                    tooltip_content += "<br><br>✅ <strong>Overstaffed</strong> - Consider reallocation"
                else:
                    tooltip_content += "<br><br>✅ <strong>Optimal staffing</strong> - No gap detected"
...
def main():
    ...
    if current_view == "Shift Summary":
        ...
        # Expandable Details Section for Shift Summary
        with st.expander("📊 View Detailed Shift Breakdowns"):
            st.markdown("**Detailed breakdown for each shift with comparisons to previous week**")
            
            # Create comprehensive shift data using filtered data
            shift_data = []
            for shift_key, data in filtered_shift_data.items():
                parts = shift_key.split(' ')
                date = parts[0]
                day = parts[1] 
                shift_num = parts[-1].split(' ')[-1]
                ...
                shift_data.append({
                    ...
                    'Day': day,
                    'Shift': shift_num,
                    'Total_Needed': data['Total Needed'],
                    'Total_Expected': data['Total Expected'],
                    'Total_Gap': data['Total Gap'],
                    'Attendance': data['Total Attendance Assumption'],
                    'Total_Punches': data['Total Punches'],
                    'Needed_change': data.get('tooltip_needed', ''),
                    'Expected_change': data.get('tooltip_expected', ''),
                    'Gap_change': data.get('tooltip_gap', ''),
                    'Attendance_change': data.get('tooltip_attendance', ''),
                    'Punches_change': data.get('tooltip_punches', '')
                })
            
            # Display by date
            current_date = None
            for shift_entry in shift_data:
                if shift_entry['Date'] != current_date:
                    current_date = shift_entry['Date']
                    st.markdown(f"### {current_date} - {shift_entry['Day']}")
                
                col1, col2 = st.columns([1, 1])
                
                with col1:
                    st.markdown(f"**Shift {shift_entry['Shift']} - Summary**")
                    
                    shift_summary_df = pd.DataFrame([
                        ['Total Needed', shift_entry['Total_Needed']],
                        ['Total Expected', shift_entry['Total_Expected']],
                        ['Total Gap', shift_entry['Total_Gap']],
                        ['Attendance %', shift_entry['Attendance']],
                        ['Total Punches', shift_entry['Total_Punches']]
                    ], columns=['Metric', 'Value'])
                    
                    st.dataframe(shift_summary_df, hide_index=True, use_container_width=True)
                
                with col2:
                    st.markdown(f"**Shift {shift_entry['Shift']} - Changes vs Last Week**")
                    
                    changes_df = pd.DataFrame([
                        ['Needed Breakdown', shift_entry['Needed_change']],
                        ['Expected Change', shift_entry['Expected_change']],
                        ['Gap Change', shift_entry['Gap_change']],
                        ['Attendance Change', shift_entry['Attendance_change']],
                        ['Punches Change', shift_entry['Punches_change']]
                    ], columns=['Detail Type', 'Change'])
        ...
        # Expandable Details Section for HC Details
        with st.expander("👥 View Detailed HC Breakdowns"):
            st.markdown("**Headcount details for each shift with week-over-week changes**")
            
            # Create comprehensive HC data using filtered data
            hc_data = []
            for shift_key, data in filtered_hc_data.items():
                parts = shift_key.split(' ')
                date = parts[0]
                day = parts[1] 
                shift_num = parts[-1].split(' ')[-1]
                ...
                hc_data.append({
                    ...
                    'FTE': data['FTE'],
                    'TEMP': data['TEMP'],
                    'NEW HIRES': data['NEW HIRES'],
                    'FLEX': data['FLEX'],
                    'WW/GS': data['WW/GS'],
                    'VEH/MEH': data['VEH/MEH'] if data['VEH/MEH'] != '' else 0,
                    'PTO': data['PTO'] if data['PTO'] != '' else 0,
                    'FTE_change': data.get('tooltip_fte', ''),
                    'TEMP_change': data.get('tooltip_temp', ''),
                    'NEWHIRES_change': data.get('tooltip_newhires', ''),
                    'FLEX_change': data.get('tooltip_flex', ''),
                    'WWGS_change': data.get('tooltip_wwgs', ''),
                    'VEHMEH_change': data.get('tooltip_vehmeh', ''),
                    'PTO_change': data.get('tooltip_pto', '')
                })
            
            # Display by date
            current_date = None
            for hc_entry in hc_data:
                if hc_entry['Date'] != current_date:
                    current_date = hc_entry['Date']
                    st.markdown(f"### {current_date} - {hc_entry['Day']}")
                
                col1, col2 = st.columns([1, 1])
                
                with col1:
                    st.markdown(f"**Shift {hc_entry['Shift']} - Headcount**")
        ...
        # Expandable Details Section for Attendance Assumptions
        with st.expander("📈 View Detailed Attendance Breakdowns"):
            st.markdown("**Attendance assumptions and show-up rates with week-over-week changes**")
            
            # Create attendance data structure using filtered data
            attendance_data = []
            for shift_key, data in filtered_attendance_data.items():
                parts = shift_key.split(' ')
                date = parts[0]
                day = parts[1]
                shift_num = parts[-1].split(' ')[-1]
                ...
                attendance_data.append({
                    ...
                    'Day': day,
                    'Shift': shift_num,
                    'FTE_Attendance': data['FTE Attendance Assumption'],
                    'TEMP_Attendance': data['TEMP Attendance Assumption'],
                    'NEW_HIRES_Rate': data['NEW HIRES Show Up Rate'],
                    'FLEX_Rate': data['FLEX Show Up Rate'],
                    'WWGS_Rate': data['WW/GS Show Up Rate'],
                    'VEH_Rate': data['VEH Show Up Rate'],
                    'FTE_change': data.get('tooltip_fte', ''),
                    'TEMP_change': data.get('tooltip_temp', ''),
                    'NEWHIRES_change': data.get('tooltip_newhires', ''),
                    'FLEX_change': data.get('tooltip_flex', ''),
                    'WWGS_change': data.get('tooltip_wwgs', ''),
                    'VEH_change': data.get('tooltip_veh', '')
                })
            
            # Display by date
            current_date = None
            for att_entry in attendance_data:
                if att_entry['Date'] != current_date:
                    current_date = att_entry['Date']
                    st.markdown(f"### {current_date} - {att_entry['Day']}")
                
                col1, col2 = st.columns([1, 1])
                
                with col1:
                    st.markdown(f"**Shift {att_entry['Shift']} - Attendance Rates**")
                    
                    att_summary_df = pd.DataFrame([
                        ['FTE Attendance', att_entry['FTE_Attendance']],
                        ['TEMP Attendance', att_entry['TEMP_Attendance']],
                        ['NEW HIRES Rate', att_entry['NEW_HIRES_Rate']],
                        ['FLEX Rate', att_entry['FLEX_Rate']],
                        ['WW/GS Rate', att_entry['WWGS_Rate']],
                        ['VEH Rate', att_entry['VEH_Rate']]
                    ], columns=['Type', 'Rate'])
                    
                    st.dataframe(att_summary_df, hide_index=True, use_container_width=True)
                
                with col2:
                    st.markdown(f"**Shift {att_entry['Shift']} - Changes vs Last Week**")
                    
                    att_changes_df = pd.DataFrame([
                        ['FTE Attendance', att_entry['FTE_change']],
                        ['TEMP Attendance', att_entry['TEMP_change']],
                        ['NEW HIRES Rate', att_entry['NEWHIRES_change']],
                        ['FLEX Rate', att_entry['FLEX_change']],
                        ['WW/GS Rate', att_entry['WWGS_change']],
                        ['VEH Rate', att_entry['VEH_change']]
                    ], columns=['Type', 'Change'])
                    
                    st.dataframe(att_changes_df, hide_index=True, use_container_width=True)
