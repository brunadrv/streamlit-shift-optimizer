...
def create_transposed_shift_summary_table_with_tooltips():
    ...
    fig.update_layout(
        height=300,
        margin=dict(l=0, r=0, t=20, b=-30),  # More aggressive bottom margin reduction
...
def create_transposed_hc_details_table_with_tooltips():
    ...
    fig.update_layout(
        height=350,
        margin=dict(l=0, r=0, t=20, b=-30),  # More aggressive margin reduction
...
def main():
    ...
    if current_view == "Shift Summary":
        ...
        # Expandable Details Section for Shift Summary (with reduced spacing)
        st.markdown('<style>div.streamlit-expanderHeader { margin-top: -20px !important; }</style>', unsafe_allow_html=True)
        with st.expander("📊 View Detailed Shift Breakdowns"):
            ...
            # Group shifts by date and display in simple format
            shifts_by_date = {}
            for shift_key in st.session_state.shift_summary_transposed.keys():
                date = shift_key.split(' ')[0]
                if date not in shifts_by_date:
                    shifts_by_date[date] = []
                shifts_by_date[date].append(shift_key)
            
            for date, shifts in shifts_by_date.items():
                st.markdown(f"### {date}")
                
                # Create columns for each shift in that date
                cols = st.columns(len(shifts))
                
                for i, shift_key in enumerate(shifts):
                    data = st.session_state.shift_summary_transposed[shift_key]
                    shift_num = shift_key.split(' ')[-1].split(' ')[-1]  # Extract "1", "2", "3"
                    
                    with cols[i]:
                        st.markdown(f"**{shift_num}**")
                        
                        # Total Needed
                        st.markdown("🔴 **Total Needed:**")
                        st.markdown(f"• Count: {data['Total Needed']}")
                        st.markdown(f"• Breakdown: {data['tooltip_needed']}")
                        
                        # Total Expected
                        st.markdown("🔵 **Total Expected:**") 
                        st.markdown(f"• Count: {data['Total Expected']}")
                        st.markdown(f"• {data['tooltip_expected']}")
                        
                        # Gap
                        gap_color = "🔴" if data['Total Gap'] < 0 else "🟢"
                        st.markdown(f"{gap_color} **Gap:** {data['Total Gap']}")
                        st.markdown(f"• {data['tooltip_gap']}")
                        
                        # Attendance
                        st.markdown(f"📊 **Attendance:** {data['Total Attendance Assumption']}")
                        st.markdown(f"• {data['tooltip_attendance']}")
                        
                        # Punches  
                        st.markdown(f"👥 **Punches:** {data['Total Punches']}")
                        st.markdown(f"• {data['tooltip_punches']}")
