...
def create_combined_hc_attendance_aggrid_table(filtered_hc_data, filtered_attendance_data, expected_hc_total):
    ...
    # Adjust column widths based on number of shifts for better readability
    num_shifts = len(columns_list)
    if num_shifts > 9:
        # Many shifts - use compact widths
        roster_width = 90
        attendance_width = 140
        expected_width = 100
        punches_width = 110
        variance_width = 150
    elif num_shifts > 6:
        # Medium number of shifts
        roster_width = 100
        attendance_width = 160
        expected_width = 110
        punches_width = 120
        variance_width = 170
    else:
        # Few shifts - use comfortable widths
        roster_width = 120
        attendance_width = 180
        expected_width = 130
        punches_width = 140
        variance_width = 200
    ...
    for idx, shift_key in enumerate(columns_list):
        ...
        columnDefs.append({
            'headerName': f'{date_str} {day_abbr} - Shift {shift_num}' if num_shifts <= 6 else f'{date_str} S{shift_num}',
...
def main():
    ...
    <div style="display: flex; gap: 20px; margin: 10px 0 20px 0; font-size: 13px; color: #666;">
        ...
        <div style="display: flex; align-items: center; gap: 8px;">
            <span>✏️ Click cells in the hedge row to increase or decrease Attendance Assumptions for FTE and TEMP</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
