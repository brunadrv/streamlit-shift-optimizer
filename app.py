...
def main():
    ...
    with emp_col2:
        employee_id_search = st.text_input("Employee ID (search)", "", key="emp_id_filter")
    
    with emp_col3:
        schedules = list(set([emp['Workday Schedule'] for emp in st.session_state.employee_data]))
        selected_schedules = st.multiselect("Workday Schedule", schedules, default=schedules, key="schedule_filter")
    
    with emp_col4:
        roster_buckets = list(set([emp['Roster Bucket'] for emp in st.session_state.employee_data]))
        selected_roster_buckets = st.multiselect("Roster Bucket", roster_buckets, default=roster_buckets, key="roster_bucket_filter")
