...
def generate_dynamic_table_data(location, department, week, selected_dates, shifts):
    ...
    for selected_date in selected_dates:
        ...
        for shift in shifts:
            ...
            # Get shift-specific multiplier
            shift_multipliers = {
                "1": 1.4,   # Shift 1 - highest headcount (40% more than base)
                "2": 1.0,   # Shift 2 - medium headcount (base level)
                "3": 0.5    # Shift 3 - lowest headcount (50% of base)
            }
            shift_mult = shift_multipliers.get(shift_num, 1.0)
            key = f"{date_str} {day_str} Shift {shift_num}"
            
            # Apply multipliers to shift data including shift variation
            ...
            # Update attendance assumption based on shift
            shift_attendance_variations = {
                "1": "92%",  # Morning shift - better attendance
                "2": "88%",  # Day shift - average attendance
                "3": "78%"   # Night shift - lower attendance
            }
            shift_data["Total Attendance Assumption"] = shift_attendance_variations.get(shift_num, "89%")
            shift_data['Total Punches'] = int(shift_data['Total Punches'] * loc_mult * dept_mult * week_mult)
            
            dynamic_shift_data[key] = shift_data
            
            # Apply multipliers to HC data including shift variation
            ...
            hc_data['FLEX'] = max(0, int(hc_data['FLEX'] * loc_mult * dept_mult * week_mult * shift_mult))
            
            dynamic_hc_data[key] = hc_data
            
            # Attendance data with shift-specific variations
            attendance_data = base_attendance_template.copy()
            shift_attendance_lookup = {
                "1": {"FTE": "92%", "TEMP": "87%"},
                "2": {"FTE": "88%", "TEMP": "82%"},
                "3": {"FTE": "78%", "TEMP": "72%"}
            }
            if shift_num in shift_attendance_lookup:
                attendance_data["FTE Attendance Assumption"] = shift_attendance_lookup[shift_num]["FTE"]
                attendance_data["TEMP Attendance Assumption"] = shift_attendance_lookup[shift_num]["TEMP"]
...
def main():
    ...
    # Calculate total expected HC from filtered data for the section title
    total_expected_hc = 0
    if filtered_shift_data:
        total_expected_hc = sum(data.get('Total Expected', 0) for data in filtered_shift_data.values())
    
    # Department Details section - responsive to department filter
    st.markdown(f'<div class="section-header">{selected_department} Expected HC ({total_expected_hc})</div>', unsafe_allow_html=True)
