    ...
    for emp_type in employee_types:
        if emp_type == 'Hedge Attendance Rate (+/-) ✏️':
            ...
            for idx, shift_key in enumerate(columns_list):
                ...
                # Calculate Expected HC = Roster × (Attendance + Hedge)
                # Get hedge rate for this shift
                ...
                # Apply hedge only to FTE and TEMP
                if emp_type in ['FTE', 'TEMP']:
                    hedged_attendance = attendance_pct + (hedge_rate / 100.0)
                    hedged_attendance = max(0, min(1, hedged_attendance))  # Clamp between 0 and 1
                else:
                    hedged_attendance = attendance_pct
                
                expected_hc = round(hc_value * hedged_attendance)
                table_data[f'{col_prefix}_Expected'].append(expected_hc)
    
    df = pd.DataFrame(table_data)
    
    # Calculate Total Expected HC row by summing Expected HC column (excluding hedge and total rows)
    total_row_idx = df[df['Employee Type'] == 'Total Expected HC'].index
    if len(total_row_idx) > 0:
        total_idx = total_row_idx[0]
        # Sum all Expected HC values for non-total, non-hedge rows
        for idx, shift_key in enumerate(columns_list):
            col_prefix = f'S{idx+1}'
