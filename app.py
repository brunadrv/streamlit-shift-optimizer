...
if 'data_initialized' not in st.session_state:
    ...
    # Punch rates closer to attendance assumptions (±3-5% variance)
    # These rates are slightly higher/lower than attendance assumptions to create realistic scenarios
    punch_rates = {
        'FTE': 0.92,          # Attendance assumption ~90%, so 92% is slightly over
        'TEMP': 0.82,         # Attendance assumption ~84%, so 82% is slightly under
        'NEW HIRES': 0.52,    # Show up rate ~50%, so 52% is slightly over
        'FLEX': 0.52,         # Show up rate ~50%, so 52% is slightly over
        'WW/GS': 0.98         # Show up rate ~100%, so 98% is slightly under
    }
...
def create_combined_hc_attendance_aggrid_table(filtered_hc_data, filtered_attendance_data, expected_hc_total):
    ...
    def calculate_punch_totals(filtered_hc_data):
        """Calculate actual punch counts by shift and employee type"""
        punch_data = {}
        
        for shift_key in filtered_hc_data.keys():
            # Parse shift key: "2026-02-12 Thu Shift 1"
            parts = shift_key.split(' ')
            date_str = parts[0]
            shift_num = parts[-1]  # "1", "2", or "3"
            
            # Map shift number to shift name
            shift_map = {'1': '1st', '2': '2nd', '3': '3rd'}
            shift_name = shift_map.get(shift_num, f"{shift_num}st")
            
            # Count punches by employee type for this shift
            shift_punches = {}
            for emp in st.session_state.employee_data:
                if emp['Date'] == date_str and emp['Shift'] == shift_name and emp['Punched_In']:
                    worker_type = emp['Worker Type']
                    shift_punches[worker_type] = shift_punches.get(worker_type, 0) + 1
            
            punch_data[shift_key] = shift_punches
        
        return punch_data
    
    punch_totals = calculate_punch_totals(filtered_hc_data)
