...
def validate_and_adjust_hc_totals(filtered_hc_data, filtered_attendance_data, overview_expected, employee_types, data_key_mapping, attendance_key_mapping):
    ...
    # Calculate current table Expected HC total (sum across all shifts)
    ...
    for shift_key in sorted(filtered_hc_data.keys()):
        ...
        for emp_type in employee_types:
            ...
            shift_expected += scheduled_hc * attendance_pct

        

        shift_expected_hcs.append((shift_key, shift_expected))

    

    table_expected_total = sum(exp for _, exp in shift_expected_hcs)

    

    # Calculate adjustment ratio

    if table_expected_total > 0:

        adjustment_ratio = overview_expected / table_expected_total
    ...
    # Adjust each shift's HC values proportionally and track total
    ...
    for i, (shift_key, _) in enumerate(shift_expected_hcs):
        ...
        # For all but the last shift, round normally

        if i < len(shift_expected_hcs) - 1:

            for emp_type in employee_types:

                data_key = data_key_mapping.get(emp_type, emp_type)

                if data_key in adjusted_hc_data[shift_key]:

                    adjusted_value = hc_data[data_key] * adjustment_ratio

                    adjusted_hc_data[shift_key][data_key] = int(round(adjusted_value))

            

            # Calculate this shift's expected HC
            ...
            for emp_type in employee_types:
                ...
                scheduled = adjusted_hc_data[shift_key].get(data_key, 0)

                att_str = attendance_data.get(attendance_key, '0%')

                att_pct = float(att_str.rstrip('%')) / 100.0

                shift_exp += scheduled * att_pct

            actual_total += round(shift_exp)

        else:

            # For the last shift, adjust to make the total exactly match

            remaining_expected = overview_expected - actual_total

            attendance_data = filtered_attendance_data[shift_key]

            

            # Calculate what scheduled HC would give us the remaining expected

            # We'll adjust proportionally across employee types

            current_shift_expected = 0
            ...
            for emp_type in employee_types:
                ...
                scheduled = hc_data[data_key]

                att_str = attendance_data.get(attendance_key, '0%')

                att_pct = float(att_str.rstrip('%')) / 100.0

                current_shift_expected += scheduled * att_pct

            

            if current_shift_expected > 0:

                last_shift_ratio = remaining_expected / current_shift_expected

            else:

                last_shift_ratio = 1.0
            ...
            for emp_type in employee_types:
                ...
                if data_key in adjusted_hc_data[shift_key]:
                    ...
                    adjusted_hc_data[shift_key][data_key] = max(0, int(round(adjusted_value)))
