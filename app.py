...
def main():
    ...
    if current_view == "Shift Summary":
        ...
        attendance_view = st.radio(
            "Choose view format:",
            ["Transposed View (Recommended)", "Original Employee Detail View"],
            key="attendance_view",
            horizontal=True
        )
        
        if attendance_view == "Transposed View (Recommended)":
            st.markdown("*Hover over values to see details - Red highlights indicate low values*")
            st.plotly_chart(create_transposed_attendance_assumptions_table(), use_container_width=True)
            
            st.markdown("**Tips for Transposed Attendance View:**")
            st.markdown("- Each column represents a Date/Day/Shift combination")
            st.markdown("- Rows show different attendance assumptions and show-up rates")
            st.markdown("- Red highlights indicate low values that may need attention")
        else:
            # Original employee detail view
