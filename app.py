...
def main():
    ...
    if current_view == "Shift Summary":
        ...
        with st.expander("📊 View Detailed Shift Breakdowns"):
            ...
            # Organize shifts by date for better readability
            shifts_by_date = {}
            for shift_key in st.session_state.shift_summary_transposed.keys():
                date = shift_key.split(' ')[0]  # Extract date part
        ...
        with st.expander("👥 View Detailed HC Breakdowns"):
            ...
            shifts_by_date = {}
            for shift_key in st.session_state.weekly_hc_details_transposed.keys():
            ...
            for date, shifts in shifts_by_date.items():
                ...
                for i, shift_key in enumerate(shifts):
                    data = st.session_state.weekly_hc_details_transposed[shift_key]
                    with cols[i]:
                        shift_name = shift_key.split(' ')[-1]
                        st.markdown(f"**{shift_name}**")
                        
                        # FTE Details
                        st.markdown(f"👔 **FTE:** {data['FTE']}")
                        if 'tooltip_fte' in data:
                            st.markdown(f"• {data['tooltip_fte']}")
                        
                        # TEMP Details
                        st.markdown(f"🔄 **TEMP:** {data['TEMP']}")
                        if 'tooltip_temp' in data:
                            st.markdown(f"• {data['tooltip_temp']}")
                        
                        # NEW HIRES Details
                        st.markdown(f"🆕 **NEW HIRES:** {data['NEW HIRES']}")
                        if 'tooltip_newhires' in data:
                            st.markdown(f"• {data['tooltip_newhires']}")
                        
                        # FLEX Details
                        st.markdown(f"💪 **FLEX:** {data['FLEX']}")
                        if 'tooltip_flex' in data:
                            st.markdown(f"• {data['tooltip_flex']}")
                        
                        # WW/GS Details
                        st.markdown(f"⚡ **WW/GS:** {data['WW/GS']}")
                        if 'tooltip_wwgs' in data:
                            st.markdown(f"• {data['tooltip_wwgs']}")
                        
                        # VEH/MEH Details
                        veh_value = data['VEH/MEH'] if data['VEH/MEH'] != '' else 0
                        color = "🟢" if veh_value == 2 else ""
                        st.markdown(f"{color} **VEH/MEH:** {veh_value}")
                        if 'tooltip_vehmeh' in data:
                            st.markdown(f"• {data['tooltip_vehmeh']}")
                        
                        # PTO Details
                        pto_value = data['PTO'] if data['PTO'] != '' else 0
                        color = "🔴" if pto_value == 1 else ""
                        st.markdown(f"{color} **PTO:** {pto_value}")
                        if 'tooltip_pto' in data:
                            st.markdown(f"• {data['tooltip_pto']}")
        ...
        with st.expander("📈 View Detailed Attendance Breakdowns"):
            ...
            # Organize shifts by date
            shifts_by_date = {}
            for shift_key in st.session_state.attendance_assumptions_transposed.keys():
                date = shift_key.split(' ')[0]
                if date not in shifts_by_date:
                    shifts_by_date[date] = []
                shifts_by_date[date].append(shift_key)
            ...
            for date, shifts in shifts_by_date.items():
                ...
                for i, shift_key in enumerate(shifts):
                    data = st.session_state.attendance_assumptions_transposed[shift_key]
                    with cols[i]:
                        shift_name = shift_key.split(' ')[-1]
                        st.markdown(f"**{shift_name}**")
                        
                        # FTE Attendance
                        st.markdown(f"👔 **FTE Attendance:** {data['FTE Attendance Assumption']}")
                        if 'tooltip_fte' in data:
                            st.markdown(f"• {data['tooltip_fte']}")
                        
                        # TEMP Attendance (highlight low values)
                        temp_rate = data['TEMP Attendance Assumption']
                        color = "🔴" if temp_rate == '60%' else ""
                        st.markdown(f"{color} **TEMP Attendance:** {temp_rate}")
                        if 'tooltip_temp' in data:
                            st.markdown(f"• {data['tooltip_temp']}")
                        
                        # NEW HIRES Show Up Rate
                        nh_rate = data['NEW HIRES Show Up Rate']
                        color = "🔴" if nh_rate == '50%' else ""
                        st.markdown(f"{color} **NEW HIRES Rate:** {nh_rate}")
                        if 'tooltip_newhires' in data:
                            st.markdown(f"• {data['tooltip_newhires']}")
                        
                        # FLEX Show Up Rate
                        flex_rate = data['FLEX Show Up Rate']
                        color = "🔴" if flex_rate == '50%' else ""
                        st.markdown(f"{color} **FLEX Rate:** {flex_rate}")
                        if 'tooltip_flex' in data:
                            st.markdown(f"• {data['tooltip_flex']}")
                        
                        # WW/GS Show Up Rate
                        st.markdown(f"⚡ **WW/GS Rate:** {data['WW/GS Show Up Rate']}")
                        if 'tooltip_wwgs' in data:
                            st.markdown(f"• {data['tooltip_wwgs']}")
                        
                        # VEH Show Up Rate
                        st.markdown(f"🚗 **VEH Rate:** {data['VEH Show Up Rate']}")
                        if 'tooltip_veh' in data:
                            st.markdown(f"• {data['tooltip_veh']}")
