...
def main():
    ...
    # Four toggle buttons with custom highlighting - Simple approach that works
    ...
    # Create styled buttons with highlighting
    for i, (col, view, icon) in enumerate(zip(columns, view_options, view_icons)):
        with col:
            # Determine if this view is selected
            is_selected = st.session_state.current_view == view
            
            # Set styling based on selection
            if is_selected:
                bg_color = "#2ca02c"  # Green for selected
                text_color = "white"
                border = "3px solid #2ca02c"
            else:
                bg_color = "#f0f2f6"  # Light gray for unselected
                text_color = "#262730"
            ...
            # Simple button for click detection
