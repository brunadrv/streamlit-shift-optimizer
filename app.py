...
def main():
    ...
    # Key metrics row - Updated to show only Expected HC and Gap in HC
    ...
    # Weekly Overview with 4 toggle buttons
    ...
    if st.session_state.current_view == "Shift Summary":
        ...
        dept_filter = st.selectbox("Filter by Department", ["All"] + list(st.session_state.departments.keys()))
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.metric("Attendance Assumption", "92%")
        with col2:
            st.metric("vs Previous Week", "-6 workers", delta="-6", delta_color="inverse")
        ...
        attendance_df = create_attendance_assumption_view()
    ...
    # Date selector as shown in mockup
    col1, col2 = st.columns(2)
    with col1:
        st.markdown("**Date**")
    ...
    with col2:
        ...
        selected_dept = st.selectbox("", list(st.session_state.departments.keys()), 
