...
def generate_shift_breakdown(filtered_shift_data, metric_key):
    """Generate shift-level breakdown text for overview widgets, grouped by day"""
    ...
    # Format each day's shifts
    day_lines = []
    for day in sorted(day_groups.keys()):
        shifts = sorted(day_groups[day], key=lambda x: int(x[0]) if x[0].isdigit() else 0)
        shift_parts = [f"Shift {shift_num}: {value}" for shift_num, value in shifts]
        day_line = f"{day} - {' • '.join(shift_parts)}"
        day_lines.append(day_line)
    
    return "<br>".join(day_lines) if day_lines else "No shift data available"
...
def main():
    ...
    with col1:
        ...
        # Shift breakdown directly below widget
        needed_breakdown = generate_shift_breakdown(filtered_shift_data, 'Total Needed')
        st.markdown(f'<div style="font-size: 0.85rem; color: #666; margin-top: 8px; line-height: 1.3;">{needed_breakdown}</div>', unsafe_allow_html=True)
    ...
    with col2:
        ...
        st.markdown(f'<div style="font-size: 0.85rem; color: #666; margin-top: 8px; line-height: 1.3;">{expected_breakdown}</div>', unsafe_allow_html=True)
    ...
    with col3:
        ...
        st.markdown(f'<div style="font-size: 0.85rem; color: #666; margin-top: 8px; line-height: 1.3;">{gap_breakdown}</div>', unsafe_allow_html=True)
    ...
    with col4:
        ...
        st.markdown(f'<div style="font-size: 0.85rem; color: #666; margin-top: 8px; line-height: 1.3;">{punches_breakdown}</div>', unsafe_allow_html=True)
