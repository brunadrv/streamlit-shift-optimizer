...
def generate_shift_breakdown(filtered_shift_data, metric_key):
    """Generate shift-level breakdown with clean gray badge formatting"""
    ...
    for shift_key in sorted(filtered_shift_data.keys()):
        ...
        if len(parts) >= 4:  # Ensure proper format: YYYY-MM-DD Day Shift N
            ...
            day_groups[day]['shifts'].append((shift_num, shift_value))
    
    # Sort days chronologically by date, not alphabetically
    sorted_days = sorted(day_groups.items(), key=lambda x: x[1]['date'])
    
    # Format each day's shifts with clean gray badges
    day_lines = []
    for day, day_data in sorted_days:
        shifts = sorted(day_data['shifts'], key=lambda x: int(x[0]) if x[0].isdigit() else 0)
        
        # Create clean gray badges for each shift
        shift_badges = []
        for shift_num, value in shifts:
            badge_html = f'<span style="background-color: #6c757d; color: white; padding: 2px 6px; border-radius: 10px; font-size: 0.75rem; margin: 0 2px; font-weight: 500;">S{shift_num}: {value}</span>'
            shift_badges.append(badge_html)
        
        # Format day line without icons, clean and simple
        day_line = f'<div style="margin: 2px 0;"><span style="font-weight: 600; color: #555; margin-right: 6px;">{day}</span>{"".join(shift_badges)}</div>'
