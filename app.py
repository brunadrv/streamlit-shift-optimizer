...
def generate_shift_breakdown(filtered_shift_data, metric_key):
    ...
    for day, day_data in sorted_days:
        ...
        # Shift badges container (equal width columns)
        day_line += f'<div style="display: flex; flex: 1; gap: 4px;">'
        
        for shift_num in sorted_shifts:
            value = day_data['shifts'].get(shift_num, '—')
            # Use lighter gray and ensure consistent badge width
            badge_html = f'<span style="background-color: #adb5bd; color: white; padding: 2px 8px; border-radius: 10px; font-size: 0.75rem; font-weight: 500; text-align: center; min-width: 45px; display: inline-block;">S{shift_num}: {value}</span>'
            day_line += f'<div style="flex: 1; display: flex; justify-content: flex-start;">{badge_html}</div>'
