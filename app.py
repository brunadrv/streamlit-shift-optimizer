...
def main():
    ...
    with col4:
        st.markdown("**Date**")
        # Generate dates based on selected week (Thursday-Wednesday production weeks)
        def get_week_dates(week_str):
            """Generate all 7 days for a given production week (Thursday-Wednesday)"""
            week_mappings = {
            ...
            return [thursday + timedelta(days=i) for i in range(7)]
        
        available_dates = get_week_dates(week)
        
        # Add "All Days" option at the top
        date_options = ["All Days"] + available_dates
        
        selected_date_options = st.multiselect(
            "", 
            options=date_options,
            default=["All Days"],
            format_func=lambda x: "All Days" if x == "All Days" else x.strftime("%a %m/%d"),
            label_visibility="collapsed"
        )
        
        # Convert selection back to actual dates
        if "All Days" in selected_date_options:
            selected_dates = available_dates
        else:
            selected_dates = [d for d in selected_date_options if d != "All Days"] 
