...
def main():
    ...
    with emp_col1:
        day_filter = st.selectbox("Day of Week", ["All"] + list(filtered_employees["Day of Week"].unique()) if not filtered_employees.empty else ["All"])
    with emp_col2:
        shift_filter = st.selectbox("Shift", ["All"] + list(filtered_employees["Shift"].unique()) if not filtered_employees.empty else ["All"])
    with emp_col3:
        worker_type_filter = st.selectbox("Worker Type", ["All"] + list(filtered_employees["Worker Type"].unique()) if not filtered_employees.empty else ["All"])
    with emp_col4:
        emp_name_filter = st.text_input("Employee Name", "")
    
    # Apply filters
    filtered_emp = filtered_employees.copy()
    if day_filter != "All":
        filtered_emp = filtered_emp[filtered_emp["Day of Week"] == day_filter]
    if shift_filter != "All":
        filtered_emp = filtered_emp[filtered_emp["Shift"] == shift_filter]
    if worker_type_filter != "All":
        filtered_emp = filtered_emp[filtered_emp["Worker Type"] == worker_type_filter]
    if emp_name_filter:
        filtered_emp = filtered_emp[filtered_emp["Employee Name"].str.contains(emp_name_filter, case=False, na=False)]
