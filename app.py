...
def generate_rollup_data(selected_dates):
    """Generate aggregated rollup data by department and shift"""
    ...
    for dept in departments:
        for shift in shifts:
            # Calculate totals across all selected dates
            total_needed = 0
            total_expected = 0
            total_punches = 0
            ...
            for date in selected_dates:
                date_str = date.strftime("%Y-%m-%d")
                # Use similar logic to calculate metrics
            ...
            expected_gap = total_expected - total_needed
            actual_gap = total_punches - total_expected
            
            rollup_data.append({
                'Department': dept,
                'Shift': shift,
                'Total Needed HC': total_needed,
                'Total Expected HC': total_expected,
                'Total Expected HC Gap': expected_gap,
                'Total Actual Punches': total_punches,
                'Total Actual HC Gap': actual_gap
            })
...
def display_rollup_table(rollup_data, selected_dates):
    """Display the rollup table using AG-Grid"""
    from st_aggrid import AgGrid, GridOptionsBuilder, GridUpdateMode, DataReturnMode, JsCode
    ...
    columnDefs = [
        {'field': 'Department', 'headerName': 'Department', 'pinned': 'left', 'width': 150},
        {'field': 'Shift', 'headerName': 'Shift', 'pinned': 'left', 'width': 100},
        {'field': 'Total Needed HC', 'headerName': 'Total Needed HC', 'width': 150},
        {'field': 'Total Expected HC', 'headerName': 'Total Expected HC', 'width': 160},
        {
            'field': 'Total Expected HC Gap',
            'headerName': 'Total Expected HC Gap',
            'width': 180,
            'cellStyle': gap_style,
            'valueFormatter': gap_formatter
        },
        {'field': 'Total Actual Punches', 'headerName': 'Total Actual Punches', 'width': 170},
        {
            'field': 'Total Actual HC Gap',
            'headerName': 'Total Actual HC Gap',
            'width': 170,
            'cellStyle': gap_style,
            'valueFormatter': gap_formatter
        }
    ...
    with col1:
        st.metric("Total Needed HC", f"{df['Total Needed HC'].sum():,}")
    with col2:
        st.metric("Total Expected HC", f"{df['Total Expected HC'].sum():,}")
    with col3:
        gap = df['Total Expected HC Gap'].sum()
        st.metric("Expected HC Gap", f"{gap:+,}")
    with col4:
        st.metric("Total Actual Punches", f"{df['Total Actual Punches'].sum():,}")
    with col5:
        actual_gap = df['Total Actual HC Gap'].sum()
        st.metric("Actual HC Gap", f"{actual_gap:+,}")
