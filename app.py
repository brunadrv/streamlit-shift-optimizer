...
def create_combined_headcount_attendance_table(filtered_hc_data, filtered_attendance_data):
    ...
    .low-variance { background-color: #d4edda !important; }
    ...
    <div style="margin: 20px 0; padding: 10px; border: 1px solid #ddd; border-radius: 5px; background-color: #f8f9fa;">
        ...
        <span style="margin-left: 10px; padding: 4px 8px; background-color: #d4edda; border-radius: 3px;"><10% variance from last week</span>
    </div>
    """
    ...
    html_content += "<tr><th></th>"
    for shift in shifts:
        html_content += "<th>Headcount</th><th>Attendance Assumption</th>"
    ...
    for emp_type in employee_types:
        ...
        for shift in shifts:
            ...
            if headcount > 0:
                # Simple color coding logic (can be enhanced)
                if headcount >= 40:
                    cell_class += " high-variance"
                elif headcount >= 20:
                    cell_class += " medium-variance"
                elif headcount > 0:
                    cell_class += " low-variance"
            
            # Headcount cell
            tooltip_hc = f"<strong>{emp_type} Headcount</strong><br>{shift}<br><br>Scheduled: {headcount}<br>Type: {emp_type}"
            ...
            # Attendance cell
            tooltip_att = f"<strong>{emp_type} Attendance</strong><br>{shift}<br><br>Rate: {attendance}<br>Expected: {expected}"
            html_content += f"""<td class='data-cell'>
                {attendance}
    ...
    # Total row (sum products)
    html_content += f"<tr><td class='row-header total-row'>Total Expected HC</td>"
    for shift in shifts:
        total = shift_totals[shift]
        tooltip_total = f"<strong>Total Expected HC</strong><br>{shift}<br><br>Sum of all products: {total}<br>(Headcount × Attendance)"
