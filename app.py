...
def create_shift_breakdown_text(filtered_shift_data, selected_dates, shifts, metric_key, filtered_hc_data=None, filtered_attendance_data=None):
    """
    ...
    For other metrics, reads from filtered_shift_data.
...
def main():
    ...
    if selected_dates and shifts:
        ...
        try:
            ...
            expected_breakdown = create_shift_breakdown_text(filtered_shift_data, selected_dates, shifts, 'Total Expected', filtered_hc_data, filtered_attendance_data)
    ...
    with emp_col1:
        if not emp_df.empty:
            day_options = ["All"] + sorted(emp_df["Day of Week"].unique().tolist())
        else:
            day_options = ["All"]
        day_filter = st.selectbox("Day of Week", day_options)
        
    with emp_col2:
        if not emp_df.empty:
            shift_options = ["All"] + sorted(emp_df["Shift"].unique().tolist())
        else:
            shift_options = ["All"]
        shift_filter = st.selectbox("Shift", shift_options)
    ...
    if day_filter != "All":
        filtered_emp_df = filtered_emp_df[filtered_emp_df["Day of Week"] == day_filter]
    if shift_filter != "All":
        filtered_emp_df = filtered_emp_df[filtered_emp_df["Shift"] == shift_filter]
    if worker_type_filter != "All":
        filtered_emp_df = filtered_emp_df[filtered_emp_df["Worker Type"] == worker_type_filter]
    if emp_name_filter:
        filtered_emp_df = filtered_emp_df[filtered_emp_df["Employee Name"].str.contains(emp_name_filter, case=False, na=False)]
    ...
    # Employee details table
    fig = create_employee_details_table_with_tooltips(selected_department, filtered_emp)
    if fig:
