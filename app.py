...
def get_gap_status_info(gap, gap_percentage):
    ...
    if gap == 0:
        ...
        return f"ðŸ“Š Overstaffed by {abs(gap_percentage)}%", "gap-positive", "#28a745"
...
def create_combined_hc_attendance_table(filtered_hc_data, filtered_attendance_data, expected_hc_total):
    ...
    for shift_key in columns:
        ...
        # Calculate total Expected HC (Scheduled HC Ã— Attendance Assumption)
        total_expected_hc = 0
        for emp_type in employee_types:
            data_key = data_key_mapping.get(emp_type, emp_type)
            attendance_key = attendance_key_mapping.get(emp_type, f'{emp_type} Attendance Assumption')
...
def create_shift_breakdown_text(filtered_shift_data, selected_dates, shifts, metric_key):
...
def main():
    ...
    # Create shift breakdowns for each metric
    needed_breakdown = create_shift_breakdown_text(filtered_shift_data, selected_dates, shifts, 'Total Needed')
    expected_breakdown = create_shift_breakdown_text(filtered_shift_data, selected_dates, shifts, 'Total Expected')
    gap_breakdown = create_shift_breakdown_text(filtered_shift_data, selected_dates, shifts, 'Total Gap')
    punches_breakdown = create_shift_breakdown_text(filtered_shift_data, selected_dates, shifts, 'Total Punches')
    ...
    with emp_col1:
        worker_types = list(set([emp['Worker Type'] for emp in st.session_state.employee_data]))
        selected_worker_types = st.multiselect("Worker Type", worker_types, default=worker_types, key="worker_type_filter")
    
    with emp_col2:
        employee_id_search = st.text_input("Employee ID", "", key="emp_id_filter", placeholder="Search...")
    
    with emp_col3:
        schedules = list(set([emp['Workday Schedule'] for emp in st.session_state.employee_data]))
        selected_schedules = st.multiselect("Schedule", schedules, default=schedules, key="schedule_filter")
    
    with emp_col4:
        roster_buckets = list(set([emp['Roster Bucket'] for emp in st.session_state.employee_data]))
        selected_roster_buckets = st.multiselect("Roster", roster_buckets, default=roster_buckets, key="roster_bucket_filter")
    
    # Filter employees with all criteria including new filters
    filtered_employees_for_display = filter_employee_data_by_selections(
        st.session_state.employee_data, 
        selected_department, 
        selected_dates, 
        shifts,
        worker_type_filter=selected_worker_types,
        employee_id_filter=employee_id_search,
        schedule_filter=selected_schedules,
        roster_bucket_filter=selected_roster_buckets
    )

    # Employee details table
    fig = create_employee_details_table_with_tooltips(selected_department, filtered_employees_for_display)
    if fig:
