...
def main():
    ...
    st.sidebar.markdown("""
    **Rollup View**  
    High-level summary by department and shift
    
    **Detailed View**  
    Full breakdown with attendance, punches, and variance
    """)
...
def rollup_view():
    """High-level rollup view showing totals by department and shift"""
    # Header section
    st.markdown('<h1 class="main-header">Labor Planning Shift Optimizer - Rollup View</h1>', unsafe_allow_html=True)
    
    st.markdown("""
    <div style="background-color: #e8f4f8; padding: 15px; border-radius: 5px; margin-bottom: 20px;">
        📊 <strong>Rollup View</strong> - High-level summary showing aggregated metrics by department and shift across selected dates.
        Switch to <strong>Detailed View</strong> for full breakdown with attendance rates and variance analysis.
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        ...
        location = st.selectbox("", ["AZ Goodyear", "IL Aurora", "AZ Phoenix", "IL Lake Zurich", "IL Burr Ridge"], 
                               index=0, label_visibility="collapsed", key="rollup_location")
    
    with col2:
        st.markdown("**Week**")
        week = st.selectbox("", ["2026-W08", "2026-W07", "2026-W09", "2026-W06", "2026-W10"], 
                           index=0, label_visibility="collapsed", key="rollup_week")
    ...
    if not selected_dates:
        st.warning("Please select at least one date to view data.")
        return
    
    # Generate rollup data
    rollup_data = generate_rollup_data(selected_dates)
    
    # Display rollup table
    st.markdown('<div class="section-header">Department Summary</div>', unsafe_allow_html=True)
    display_rollup_table(rollup_data, selected_dates)


def generate_rollup_data(selected_dates):
    """Generate aggregated rollup data by department, shift, and date"""
    ...
    for dept in departments:
        dept_rows = []
        dept_totals = {date.strftime("%Y-%m-%d"): {'needed': 0, 'expected': 0, 'punches': 0} for date in selected_dates}
        
        for shift in shifts:
            row = {'Department': dept, 'Shift': shift}
            ...
            for date in selected_dates:
                ...
                import random
                random.seed(hash(f"{dept}_{shift}_{date_str}"))
                
                dept_mult = st.session_state.departments[dept]
                shift_mult = {'1st': 1.2, '2nd': 1.0, '3rd': 0.8}.get(shift, 1.0)
                
                base_expected = int(100 * dept_mult * shift_mult)
                expected = base_expected + random.randint(-5, 5)
                needed = int(expected * random.uniform(1.20, 1.25))
                punches = int(expected * random.uniform(0.98, 1.02))
                
                expected_gap = expected - needed
                actual_gap = punches - expected
                
                row[f'{date_key}_Needed'] = needed
                row[f'{date_key}_Expected'] = expected
                row[f'{date_key}_Expected_Gap'] = expected_gap
                row[f'{date_key}_Punches'] = punches
                row[f'{date_key}_Actual_Gap'] = actual_gap
                
                # Accumulate for department totals
                dept_totals[date_str]['needed'] += needed
                dept_totals[date_str]['expected'] += expected
                dept_totals[date_str]['punches'] += punches
            
            dept_rows.append(row)
        
        # Add all shift rows for this department
        rollup_data.extend(dept_rows)
        
        # Add total row for this department
        total_row = {'Department': f'{dept} - Total', 'Shift': ''}
        for date in selected_dates:
            date_str = date.strftime("%Y-%m-%d")
            date_key = date.strftime("%Y-%m-%d %a")
            
            needed = dept_totals[date_str]['needed']
            expected = dept_totals[date_str]['expected']
            punches = dept_totals[date_str]['punches']
            
            total_row[f'{date_key}_Needed'] = needed
            total_row[f'{date_key}_Expected'] = expected
            total_row[f'{date_key}_Expected_Gap'] = expected - needed
            total_row[f'{date_key}_Punches'] = punches
            total_row[f'{date_key}_Actual_Gap'] = punches - expected
        
        rollup_data.append(total_row)
    
    return rollup_data


def display_rollup_table(rollup_data, selected_dates):
    """Display the rollup table using AG-Grid with department color coding and date columns"""
    from st_aggrid import AgGrid, GridOptionsBuilder, GridUpdateMode, DataReturnMode, JsCode
    
    df = pd.DataFrame(rollup_data)
    
    # Department colors for visual separation
    dept_colors = {
        'Kitchen': '#fff3e0',
        'Production': '#e8f5e9',
        'Sanitation': '#e3f2fd',
    ...
    # Row style with department color coding and total row highlighting
    row_style = JsCode("""
    function(params) {
        ...
        if (dept && deptColors[dept]) {
            style['background-color'] = deptColors[dept];
        }
    ...
    # Build column definitions with date groupings
    columnDefs = [
        {
            'field': 'Department',
            ...
            'cellStyle': JsCode("""
            function(params) {
                var style = {'font-weight': '500'};
                if (params.value && params.value.includes('- Total')) {
    ...
    # Add date column groups
    for date in selected_dates:
        date_key = date.strftime("%Y-%m-%d %a")
        
        children_cols = [
            {
                'field': f'{date_key}_Needed',
                'headerName': 'Needed HC',
                'cellStyle': data_cell_style,
                'minWidth': 100,
                'flex': 1
            },
            {
                'field': f'{date_key}_Expected',
                'headerName': 'Expected HC',
                'cellStyle': data_cell_style,
                'minWidth': 110,
                'flex': 1
            ...
            {
                'field': f'{date_key}_Expected_Gap',
                'headerName': 'Expected Gap',
                'cellStyle': gap_style,
                'valueFormatter': gap_formatter,
                'minWidth': 110,
                'flex': 1
            },
            {
                'field': f'{date_key}_Punches',
                'headerName': 'Actual Punches',
                'cellStyle': data_cell_style,
                'minWidth': 120,
                'flex': 1
            ...
            {
                'field': f'{date_key}_Actual_Gap',
                'headerName': 'Actual Gap',
                'cellStyle': gap_style,
                'valueFormatter': gap_formatter,
                'minWidth': 100,
                'flex': 1
    ...
    # Calculate height
    grid_height = 45 + (len(rollup_data) * 40) + 50
    
    # Add note for horizontal scrolling
    if len(selected_dates) > 3:
        st.markdown("*Scroll horizontally to see all dates →*")
    ...
    # Summary totals - calculate grand totals across all departments and dates
    st.markdown("---")
    st.markdown("### Grand Totals (All Departments, All Dates)")
    ...
    for date in selected_dates:
        date_key = date.strftime("%Y-%m-%d %a")
        # Only sum non-total rows
        dept_rows = df[~df['Department'].str.contains('- Total', na=False)]
        total_needed += dept_rows[f'{date_key}_Needed'].sum()
        total_expected += dept_rows[f'{date_key}_Expected'].sum()
        total_punches += dept_rows[f'{date_key}_Punches'].sum()
    
    col1, col2, col3, col4, col5 = st.columns(5)
    
    with col1:
        st.metric("Total Needed HC", f"{int(total_needed):,}")
    with col2:
        st.metric("Total Expected HC", f"{int(total_expected):,}")
    with col3:
        gap = total_expected - total_needed
        st.metric("Expected HC Gap", f"{int(gap):+,}")
    with col4:
        st.metric("Total Actual Punches", f"{int(total_punches):,}")
    with col5:
        actual_gap = total_punches - total_expected
        st.metric("Actual HC Gap", f"{int(actual_gap):+,}")


def detailed_view():
    """Detailed view with full breakdown - existing prototype"""
    # Header section - matching mockup layout
    st.markdown('<h1 class="main-header">Labor Planning Shift Optimizer - Detailed View</h1>', unsafe_allow_html=True)
    ...
    st.markdown(f'<div class="section-header">{selected_department} Shift Details ({metrics["expected"]})</div>', unsafe_allow_html=True)
