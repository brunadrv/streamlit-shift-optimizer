...
def create_combined_hc_attendance_aggrid_table(filtered_hc_data, filtered_attendance_data, expected_hc_total):
    ...
    for idx, shift_key in enumerate(columns_list):
        ...
        table_data[f'{col_prefix}_Scheduled'] = []
        table_data[f'{col_prefix}_Attendance'] = []
        table_data[f'{col_prefix}_Variance'] = []
    ...
    for emp_type in employee_types:
        if emp_type == 'Hedge Attendance Rate (+/-) ✏️':
            ...
            for idx, shift_key in enumerate(columns_list):
                ...
                col_prefix = f'S{idx+1}'
                table_data[f'{col_prefix}_Scheduled'].append(hedge_rate)
            ...
            for idx, shift_key in enumerate(columns_list):
                ...
                # Calculate total Expected HC with hedge
                parts = shift_key.split(' ')
                date_str = parts[0]
                shift_num = parts[-1]
                hedge_key = f"{date_str}_{shift_num}st"
                ...
                total_expected_hc = 0
                for et in employee_types[:7]:  # Exclude hedge and total rows
                    data_key = data_key_mapping.get(et, et)
                    att_key = attendance_key_mapping.get(et, f'{et} Attendance Assumption')
                    scheduled_hc = hc_data.get(data_key, 0)
                    attendance_str = attendance_data.get(att_key, '0%')
                    attendance_pct = float(attendance_str.rstrip('%')) / 100.0
                    
                    # Apply hedge to FTE and TEMP
                    if et in ['FTE', 'TEMP']:
                        hedged_att = attendance_pct + (hedge_rate / 100.0)
                        hedged_att = max(0, min(1, hedged_att))
                    else:
                        hedged_att = attendance_pct
                    
                    total_expected_hc += scheduled_hc * hedged_att
                
                col_prefix = f'S{idx+1}'
                table_data[f'{col_prefix}_Scheduled'].append(round(total_expected_hc))
                table_data[f'{col_prefix}_Attendance'].append('')
                table_data[f'{col_prefix}_Variance'].append(None)
                table_data[f'{col_prefix}_Tooltip'].append('')
                table_data[f'{col_prefix}_Attendance_Tooltip'].append('')
            ...
            for idx, shift_key in enumerate(columns_list):
                ...
                if variance is not None:
                    ...
                    # Generate realistic cohort breakdown (distribute change across worker types)
                    # This simulates which cohorts are driving the variance
                    cohort_breakdown = {}
                    if emp_type == 'FTE':
                        # FTE changes tend to be more stable
                        cohort_breakdown = {'FTE': round(absolute_change * 0.9), 'Others': round(absolute_change * 0.1)}
                    elif emp_type == 'TEMP':
                        # TEMP changes are more volatile
                        cohort_breakdown = {'TEMP': round(absolute_change * 0.85), 'Others': round(absolute_change * 0.15)}
                    else:
                        cohort_breakdown = {emp_type: absolute_change}
                else:
                ...
                col_prefix = f'S{idx+1}'
                table_data[f'{col_prefix}_Scheduled'].append(hc_value)
                table_data[f'{col_prefix}_Attendance'].append(attendance_value)
    ...
    function(params) {
        ...
        // Get variance for this shift
        if (field === 'S1_Scheduled') {
            variance = params.data['S1_Variance'];
        } else if (field === 'S2_Scheduled') {
            variance = params.data['S2_Variance'];
        } else if (field === 'S3_Scheduled') {
        ...
        // Apply HIGH CONTRAST variance colors
        if (variance !== null && variance !== undefined && !isNaN(variance)) {
            ...
            if (absVariance > 20) {
                ...
                style['background-color'] = '#ffcc66';  // Medium - yellow-orange
                style['color'] = '#000000';  // Black text for contrast
            }
        }
    ...
    function(params) {
        ...
        var current = params.data[shiftNum + '_Current_HC'];
        var previous = params.data[shiftNum + '_Previous_HC'];
        var absolute = params.data[shiftNum + '_Absolute_Change'];
        var variance = params.data[shiftNum + '_Variance'];
        ...
        return '📊 Variance Breakdown\\n' +
               '━━━━━━━━━━━━━━━━━━━━━━━━━━━━\\n' +
               'Current Week: ' + current + ' HC\\n' +
               'Previous Week: ' + previous + ' HC\\n' +
               'Change: ' + sign + absolute + ' HC (' + varSign + variance.toFixed(1) + '%)' +
               cohortBreakdown;
    }
    """)
    ...
    for idx, shift_key in enumerate(columns_list):
        ...
        columnDefs.append({
            ...
            'children': [
                {
                    'field': f'{col_prefix}_Scheduled',
                    'headerName': 'Scheduled Headcount',
...
def main():
    ...
    <div style="display: flex; gap: 20px; margin: 10px 0 20px 0; font-size: 13px; color: #666;">
        ...
        <div style="display: flex; align-items: center; gap: 8px;">
            <div style="width: 20px; height: 20px; border: 2px solid #cc9900; background-color: #ffcc66;"></div>
            <span>🟡 <strong>Medium</strong> (5-10% variance)</span>
        </div>
        <div style="display: flex; align-items: center; gap: 8px;">
    ...
    df, gridOptions, custom_css, grid_height = create_combined_hc_attendance_aggrid_table(
