...
def generate_dynamic_table_data(location, department, week, selected_dates, shifts):
    ...
    # Calculate sum of shift multipliers for proportional distribution
    total_shift_mult = sum(shift_multipliers.values())
    ...
    for selected_date in selected_dates:
        ...
        for shift in shifts:
            ...
            # Get shift multiplier for proportional distribution
            shift_mult = shift_multipliers.get(shift, 1.0)
            shift_proportion = shift_mult / total_shift_mult
            
            # Apply multipliers to HC data with proportional distribution
            hc_data = base_hc_template.copy()
            hc_data['FTE'] = int(hc_data['FTE'] * loc_mult * dept_mult * week_mult * shift_proportion)
            hc_data['TEMP'] = int(hc_data['TEMP'] * loc_mult * dept_mult * week_mult * shift_proportion)
            hc_data['FLEX'] = int(hc_data['FLEX'] * loc_mult * dept_mult * week_mult * shift_proportion)
