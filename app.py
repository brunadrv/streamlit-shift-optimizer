...
def generate_shift_breakdown(filtered_shift_data, metric_key):
    """Generate shift-level breakdown text for overview widgets"""
    ...
    for shift_key in sorted(filtered_shift_data.keys()):
        ...
        if len(parts) >= 4:  # Ensure proper format: YYYY-MM-DD Day Shift N
            ...
            breakdown_lines.append(f"{day} S{shift_num}: {shift_value}")
    
    return "<br>".join(breakdown_lines) if breakdown_lines else "No shift data available"
...
def main():
    ...
    with col1:
        # Generate shift breakdown for tooltip
        needed_breakdown = generate_shift_breakdown(filtered_shift_data, 'Total Needed')
        tooltip_text = f"Target HC to fulfill orders<br><br><strong>Shift Breakdown:</strong><br>{needed_breakdown}"
        render_metric_with_tooltip("Needed HC", tooltip_text)
    ...
    with col2:
        ...
        tooltip_text = f"Scheduled x Attendance Assumption<br><br><strong>Shift Breakdown:</strong><br>{expected_breakdown}"
        render_metric_with_tooltip("Expected HC", tooltip_text)
    ...
    with col3:
        ...
        tooltip_text = f"Expected - Needed<br><br><strong>Shift Breakdown:</strong><br>{gap_breakdown}"
        render_metric_with_tooltip("Gap in HC", tooltip_text)
    ...
    with col4:
        ...
        tooltip_text = f"Actual employee check-ins<br><br><strong>Shift Breakdown:</strong><br>{punches_breakdown}"
        render_metric_with_tooltip("Total Punches", tooltip_text)
